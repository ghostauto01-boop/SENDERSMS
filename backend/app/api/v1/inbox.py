"""Inbox API — conversations, messages. Polling removed — use webhooks instead."""
import logging
...
async def list_conversations(
    page: int=1, per_page: int=100, status: Optional[str]=None,
    search: Optional[str]=None, campaign_id: Optional[int]=None,
    ...
    for conv in conversations:
        ...
        items.append({
            ...
            "last_message_at": conv.last_message_at.isoformat() if conv.last_message_at else None, "created_at": conv.created_at.isoformat(),
...
async def webhook_info():
    return {
        ...
        "note": "SMS-Gate.app cloud API does not support inbox polling. Configure this webhook URL in your SMS-Gate.app Android app: Settings → Webhooks → Add Webhook. Select all events (sms:received, sms:sent, sms:delivered, sms:failed). Messages will appear in your inbox instantly."
    }
