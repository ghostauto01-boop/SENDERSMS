"""SMS Service — send, receive, Pushover with full debug logging."""
...
class SMSService:
    ...
    @staticmethod
    def _get_sim():
        try: p=os.path.join(os.path.dirname(__file__),"..","..","..",".sim_number"); return int(open(p).read().strip())
        except: return 1

    @staticmethod
    def _provider():
        return SMSGateProvider(base_url=settings.SMSGATE_BASE_URL or "https://api.sms-gate.app/3rdparty/v1",
            username=settings.SMSGATE_USERNAME or "", password=settings.SMSGATE_PASSWORD or "",
            sim_number=SMSService._get_sim(), timeout=45)
    ...
    async def send_message(self, contact_id, body, campaign_id=None):
        ...
        p=self._provider(); r=await p.send_sms(to_number=c.phone_number,message=body,idempotency_key=ik)
        if r.success: msg.status="sent"; msg.provider_message_id=r.provider_message_id; msg.sent_at=datetime.now(timezone.utc)
        else: msg.status="failed"; msg.last_error=r.error; msg.failed_at=datetime.now(timezone.utc)
        msg.provider_response=json.dumps(r.raw_response) if r.raw_response else None
        ...
        await p.close(); await self.db.flush(); return msg
    ...
    async def process_inbound_message(self, from_number, body, webhook_data=None):
        ...
        logger.info(f"INBOUND: from={n} text={body[:80]}...")
    ...
    async def _notify_inbound(self, contact, body):
        try:
            ...
            if not prov: logger.info("NOTIFY: Pushover disabled"); return
            cfg=json.loads(prov.config_json or "{}")
            uk=decrypt_value(cfg.get("user_key_encrypted",""))
            at=decrypt_value(cfg.get("app_token_encrypted",""))
            if not uk or not at: logger.warning("NOTIFY: missing user_key or app_token"); return
            from app.providers.pushover import PushoverProvider
            pp=PushoverProvider(app_token=at,user_key=uk)
            name=contact.business_name or contact.first_name or contact.phone_number
            ok=await pp.send_notification(f"📱 New SMS from {name}", body)
            logger.info(f"NOTIFY: Pushover sent={'OK' if ok else 'FAIL'} to {name}")
        except Exception as e:
            logger.error(f"NOTIFY error: {e}")

    async def check_gateway_health(self):
        p=self._provider(); h=await p.health_check(); await p.close()
        return {"is_healthy":h.is_healthy,"status":h.status,"error":h.error}
