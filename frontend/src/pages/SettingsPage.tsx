...
import { Wifi, Bell, Shield, Clock, TestTube, Eye, EyeOff, Activity, CheckCircle, XCircle } from "lucide-react";
...
function GatewayTab() {
  ...
  useEffect(() => {
    api.get("/settings/gateway").then(({data}:any) => { if(data?.sim_number) setSim(data.sim_number); }).catch(()=>{});
  }, []);
  ...
  return (
    <div className="card p-6 space-y-4 max-w-lg">
      ...
      <p className="text-xs text-gray-500">Connected to <code className="text-xs bg-gray-100 dark:bg-gray-700 px-1 rounded">api.sms-gate.app/3rdparty/v1</code>. Username: <strong>_O48UB</strong>.</p>
